...
class LineupsRepository implements LineupsInterface {
  LineupsRepository(
    this.lineupsRemoteDataSource,
  );

  final LineupsRemoteDataSource lineupsRemoteDataSource;

  @override
  Future<ResponseWrapper<List<LineupItem>>> getFixtureLineups(
      int? fixtureId) async {
    try {
      Response responses = await lineupsRemoteDataSource.getFixtureLineups(
        fixtureId,
      );
